...
## Test Checklist
- [ ] App builds successfully (iOS incl. new NSE target; watchOS via xcodebuild)
...
- [ ] Write ADR-006 (server as privacy-neutral alarm clock; visible push + NSE for deadlines; silent push as additive wake source; rejected alternatives)
- [ ] Add App Group entitlement to iOS app (+ widget/LA extensions if needed); migrate `AutomationManager` state/parameters and `AutomationLogManager` storage to the App Group with one-shot migration
- [ ] Add `Solar Lens iOS NotificationService` target with Keychain access group + App Group; keep dependencies to `Shared/` only
- [ ] Extract a UI-free `runDeadlineTick` path from `AutomationAutoResetChargingMode` / `AutomationManager` reusable by the NSE; add a shared-store lease to avoid double execution
- [ ] NSE: load token + state, run tick, persist state + log, rewrite notification content, cancel pending local fallback notification; honest default payload text
- [ ] Verify with `xcrun simctl push` (simulator) and a manual APNs push (device, incl. force-quit)

### Slice 2 — Server scheduler (extend Solar Lens .NET Azure Functions)
- [ ] Add `Extensions.Tables`, `Extensions.Storage.Queues`, `Extensions.Timer` packages; Azurite table/queue for local dev
- [ ] `WakeScheduleService` (Azure Table `WakeSchedules`, install-secret hashing, validation/clamping of cadence/until, token format check)
- [ ] HTTP functions `PUT/DELETE/GET /api/wake/…` with `RateLimitService`
- [ ] Timer function (`0 * * * * *`): select due rows (with 15-min catch-up), enqueue one message per push, delete/advance rows, daily defensive cleanup
- [ ] Queue function: `ApnsClient` (HTTP/2, ES256 JWT cache, sandbox/production hosts, headers incl. `apns-expiration`/`collapse-id`), row re-check for idempotency; invalid token → delete device rows, no retry; transient → self re-enqueue with `visibilityTimeout` until `fireAt + 10 min`, then drop with `Warning`; unexpected → throw → poison queue; one message = one push (no batching)
- [ ] Timer function logs `apns-push-poison` depth at `Warning` when > 0; App Insights alert rule on that log; README section "what to do when the poison queue is not empty" (inspect in Portal, fix root cause, purge)
- [ ] `host.json`: `queues.maxPollingInterval` ~5 s, timer function log level `Warning`; App Insights sampling kept on
- [ ] Secrets via Function App settings (optionally Key Vault); `local.settings.json` template; server README (architecture, deployment, key rotation, cost expectations)
