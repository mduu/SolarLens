...
- [x] **Verified again after the scheduler rework (01.09.2026)**: with the app force-quit, the push was sent at 18:47:59.8 for a reset due at 18:48:00 and the banner read "Lademodus auf Nur Solar zurückgesetzt" — the extension did the work before the notification appeared. Table and queue emptied themselves, the poison queue was never created
- [x] That measurement exposed a real edge: Azure rounds queue visibility to whole seconds, so a push can surface a fraction *before* its time, and the device's clock is not the server's. The extension's `now >= resetAt` check would then have found the run "not due" and shown the fallback text instead of acting. `AutoResetCompletion` now accepts a 5-second tolerance — a reset seconds early is unnoticeable, one that silently does not happen is the failure this story exists to remove
- [x] **Production APNs verified via TestFlight (01.09.2026, build 4.5.0/355).** The app registered `environment: production`, the push was sent 0.4 s after the scheduled second and delivered, the extension applied the reset with the app force-quit, and the deadline row deleted itself. The signed distribution build was checked before upload: `aps-environment = production` (Xcode substitutes it at export), app group and shared keychain group present on both the app and the extension
- [x] **Token rotation verified as a side effect**: installing the TestFlight build over the Xcode build swapped a sandbox token for a production one. The app noticed, deleted every schedule belonging to the old token and re-registered under the new one without anyone doing anything — the same path that migrates a running automation across an app update
- [x] **Upgrade path**: a run started on an older version keeps going and gains the push. Its state is migrated out of `UserDefaults.standard` into the App Group on first read, and the first launch after the update looks like a token change, which re-registers the deadline and the silent-push window. If the user never opens the app, the run continues exactly as before on BG tasks — no regression, no benefit until they do; iOS gives no way around that
- [x] **Silent-push wakes verified on device the hard way (03.09.2026)**: TestFlight testing surfaced monitor notifications delivered twice, seconds apart, and logged twice. Telemetry confirmed the cause end to end — 187 silent deliveries in 24 h where a 15-minute window yields 96, with 91 arriving 0 s after their predecessor
- [x] Three defects fixed for that: a window *renewal* enqueued a second message for the already-queued slot (the sender's staleness check cannot tell them apart — both carry the row's exact fire time); `AdvanceWindowAsync` wrote with `ETag.All`, so two workers processing the same slot in parallel both advanced the row and both queued the next occurrence, turning one duplicate into a duplicate of every following slot; and the app's `runOverdueMonitorsInBackground` had no reentrancy guard, so two near-simultaneous wakes interleaved at the network fetch, both saw the monitor still armed, and both fired — two banners, two log entries
- [x] `DailyHousekeeping` also repairs schedules whose queue message went missing (made possible by skip-on-unchanged enqueues): a stale deadline is dropped, an overdue window is re-timed and re-queued
- [ ] Still open on device: the NSE-timeout path and the notifications-disabled fallback
- [x] Refinement noticed during the TestFlight round: the foreground path called `resyncWakeSchedule()` but not `WakeWindowCoordinator.refresh()`, so after a long force quit the silent-push window was only renewed on the next monitor tick rather than immediately. Fixed in build 361, together with the same gap in the **Settings toggle**, whose "on" branch also called `resyncWakeSchedule()` only — and that returns immediately without an active automation, so a device whose only reason for a window is a notification monitor had to wait for the next monitor tick instead of registering there and then. Measured on the real device on 06.09.2026: the row was gone at 14:35:42 and back at 14:37:25, so roughly 100 seconds, not the hours the foreground case can cost. Turning the toggle off now also drops the local `registeredUntil`, so the coordinator's view matches the server it just cleared
