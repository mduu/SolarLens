...
- [x] Refinement noticed during the TestFlight round: the foreground path called `resyncWakeSchedule()` but not `WakeWindowCoordinator.refresh()`, so after a long force quit the silent-push window was only renewed on the next monitor tick rather than immediately. Fixed in build 361, and testing it surfaced the same gap twice more: the **Settings toggle** re-registered via `resyncWakeSchedule()` only, which returns immediately without an active automation — so a device whose only reason for a window is a notification monitor was left with nothing on the server after toggling off and on. Worse, turning the toggle off cleared the server but not `registeredUntil`, so the coordinator believed it was still registered and declined to register again until the stale window expired. Off now invalidates the local record first
