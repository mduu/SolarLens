...
- [ ] Refinement noticed during the TestFlight round: the foreground path calls `resyncWakeSchedule()` but not `WakeWindowCoordinator.refresh()`, so after a long force quit the silent-push window is only renewed on the next monitor tick rather than immediately. Not a regression, deliberately left out of build 355 rather than discarding a verified archive
